...
- 当前 Phase 1–3 已实现领域模型、只读检测、安全 staging、集合预检、封印计划、绑定确认、正式一次性安装入口、原生安装、静态读回和有限补偿；并已在临时 `HOME`/`HERMES_HOME` 中通过 Hermes v0.20 的双 Profile 安装、读回、故障补偿与不确定残留演练。独立卸载行为、Phase 5 完整真实验收和发布仍未实现。
