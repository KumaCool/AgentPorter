...
- 当前 Phase 1–2 已实现领域模型、Hermes 环境只读检测、纯 staging 渲染与安全扫描、集合预检、封印计划、非秘密 provider 选择及绑定确认；并已在临时 `HOME`/`HERMES_HOME` 中通过 Hermes v0.20 原生安装契约探针。正式安装事务、独立卸载行为、完整真实验收和发布仍未实现。
