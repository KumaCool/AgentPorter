...
## 0. 当前 Adapter 状态与 0.1.5 方向

0.1.4 已正式发布，其静态三 Profile生命周期和离线绑定合同仍是基线。实际安装暴露出两个 AgentPorter侧缺口：bootstrap未发布公共 `agentporter` / `agentporter-activate`，正式 activation又固定返回 `probe-unsupported`，导致两个 Worker仍缺 provider/endpoint/Profile-local凭据并在真实调用前失败。

当前权威设计是[0.1.5 运行激活与真实调用闭环](05-runtime-activation-and-live-call-design.md)：只使用 Hermes v0.20 已公开的 Profile-scoped auth、one-shot和usage报告，不修改 Hermes源码。usage可验证 model/provider/api_calls，但缺少 tool_calls/fallback遥测，因此成功调用先报告 `live-call-passed + route-proof-incomplete`；revision-safe Kanban mutation与完整 `operational` 仍保持 unsupported。
