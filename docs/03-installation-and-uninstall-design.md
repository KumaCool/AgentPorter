...
- **代码状态：** Phase 1–3 的领域模型、只读检测、安全 staging、集合预检、封印计划、确认、正式原生安装、逐项静态读回、集合验证及有限反向补偿已实现；独立卸载行为尚未实现；
- **真实验收：** 已在临时根下完成 Hermes v0.20 双 Profile 安装、description 与 artifact 读回、第二项失败补偿第一项、命令后不确定残留不自动删除的 Phase 3 演练；Phase 5 完整 INS/UN/GATE 验收仍未执行；
