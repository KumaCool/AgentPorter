...
`product_id` 与两个 `component_id` 是代码和测试中的协议常量。实现后的唯一权威注册表是 `src/agentporter/identity.py`；测试和文档只能引用该注册表，不能各自复制另一组值。实际 UUID 在 Phase 1 生成一次并冻结，不能因 Profile、Worker、模型或显示名称变化而变化。在该注册表落地前，本设计只使用占位符，不宣称已有正式 UUID。
