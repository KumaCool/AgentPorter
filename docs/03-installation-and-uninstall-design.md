...
- **代码状态：** Phase 1–2 的领域模型、只读检测、纯渲染、安全扫描、集合预检、封印计划、非秘密 provider 选择、单次确认与确认前后重验已实现；正式安装事务与独立卸载行为尚未实现；
- **真实验收：** 仅完成临时根下 Hermes v0.20 原生 distribution 安装契约探针；正式入口及完整 INS/UN/GATE 验收未执行；
