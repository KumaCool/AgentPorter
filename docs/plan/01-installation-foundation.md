...
**当前维护状态：** 名称无关发现、封印计划、完整警告、绑定确认、集合/逐项重验、原生删除、双重读回、独立入口及临时根 Hermes v0.20 批量改名卸载演练已完成；后续维护已补充发布版完整自清理：Profile 全部删除或已不存在后，安全删除精确公开入口、当前版本私有环境与空产品目录，源码运行和其它版本保持不变。
...
8. 发布版入口在 Profile 删除成功/已不存在后封印并重验自身布局，隔离删除当前版本软件包与公开链接；失败/部分删除、源码 checkout、其它版本及身份漂移均零软件包删除。

### 验收映射

关闭 `UN-01`–`UN-10`；为 `GATE-02`–`GATE-05` 提供卸载路径证据。测试必须覆盖批量改名、所有异常集合、路径/inode 变化、非规范 basename 和原生删除后的结果分层。
