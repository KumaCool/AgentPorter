...
`hermes config check` 仅证明静态配置可解析。0.1.4 已正式发布，但实际安装后公共 activation与真实调用仍未闭合；当前修复权威见 [Plan 05](05-runtime-activation-and-live-call-closure.md)。本历史计划不声称 `operational`、真实 canary 或 live routing passed。
