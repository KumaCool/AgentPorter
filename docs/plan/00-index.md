...
| 06 | [职责型 Worker 身份与自定义推理绑定计划](06-role-identities-and-configurable-model-binding.md) | v0.2.0 已正式发布；离线、tag/release、7 个托管 assets 与外部读回闭合；live 未执行 | 用职责型名称替代模型语义名称，保持 component UUID/职责不变，并让三个 Profile 的 model/provider/endpoint 由用户显式配置 |
...
Plan 05 运行激活与真实调用闭环（0.1.5–0.1.8 已交付的运行绑定基础）
    ↓
Plan 06 职责型身份与自定义推理绑定（v0.2.0 已正式发布并完成托管读回）
    ↓
...
- Plan 06 是 v0.2.0 正式发布版的设计与实施权威：永久 component UUID 和三个职责保持不变；当前模型语义名称迁移为 `bounded_worker` / `mechanical_worker` 及对应职责型 Profile 名；三个 Profile 的 model/provider/endpoint 改为用户显式绑定。v0.2.0 已实现这些目标：fresh install 使用职责名，三个 Profile 在 staging 前显式封闭绑定；旧默认名经 `agentporter-activate` 独立确认的 Hermes-native journaled rename 迁移，用户改名保留，绑定变化使 readiness 失效。
...
- v0.2.0 tag、非预发布 GitHub Release、7 个托管 assets 和外部读回已闭合；真实模型调用、Gateway 服务变更与 Kanban mutation/live routing 仍未执行。
