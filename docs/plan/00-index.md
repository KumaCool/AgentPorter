...
| 06 | [职责型 Worker 身份与自定义推理绑定计划](06-role-identities-and-configurable-model-binding.md) | v0.2.0 发布候选已准备；离线代码与门禁闭合；未 tag/release/托管读回 | 用职责型名称替代模型语义名称，保持 component UUID/职责不变，并让三个 Profile 的 model/provider/endpoint 由用户显式配置 |
...
Plan 05 运行激活与真实调用闭环（0.1.5–0.1.8 已交付的运行绑定基础）
    ↓
Plan 06 职责型身份与自定义推理绑定（v0.2.0 发布候选已准备，待远端 CI/tag/release）
    ↓
...
- 离线开发、测试、提交和既定阶段交付持续执行；真实模型调用、Gateway 服务变更与 Kanban mutation 仍未执行；tag、release 和托管制品读回留待发布阶段。
