...
def validate_spec(spec: dict[str, Any]) -> None:
    ...
    if Path(spec["journal"]) != new.parent / ".0.2.0-entry-transaction.json":
...
def build_plan(spec: dict[str, Any], spec_seal: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    return {
        ...
        "spec": spec_seal,
        "committed": False,
        "old_root_quarantine": str(Path(spec["old_root"]).with_name(".0.1.8-txn-quarantine")),
        "new_root_quarantine": str(Path(spec["new_root"]).with_name(".0.2.0-txn-quarantine")),
