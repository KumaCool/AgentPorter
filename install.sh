#!/bin/sh
# AgentPorter v0.1.8 release-candidate bootstrap for POSIX systems.
set -eu

VERSION=0.1.8
PREVIOUS_VERSION=0.1.4
RELEASE_BASE_URL=https://github.com/KumaCool/AgentPorter/releases/download/v0.1.8
...
# The v0.1.8 transaction helper is also shipped inside the wheel/sdist; the
# standalone checksum-verified copy below is the pre-install bootstrap asset.

PREVIOUS_VERSION=0.1.6
INSTALL_ROOT=${PRODUCT_ROOT}/${VERSION}
OLD_ROOT=${PRODUCT_ROOT}/${PREVIOUS_VERSION}
JOURNAL=${PRODUCT_ROOT}/.0.1.8-entry-transaction.json
SPEC=${PRODUCT_ROOT}/.0.1.8-entry-spec.json
TXN_HELPER=bootstrap_txn.py
TXN_HELPER_CHECKSUM=${TXN_HELPER}.sha256

recover_entry_transaction() {
    [ -e "$JOURNAL" ] || [ -L "$JOURNAL" ] || return 0
    [ -f "$SPEC" ] && [ ! -L "$SPEC" ] || fail 'partial/mixed interrupted 0.1.8 install; residue retained'
    [ -f "$INSTALL_ROOT/$TXN_HELPER" ] && [ ! -L "$INSTALL_ROOT/$TXN_HELPER" ] \
        || fail 'partial/mixed interrupted 0.1.8 install; residue retained'
    "$PYTHON" "$INSTALL_ROOT/$TXN_HELPER" recover "$SPEC" \
        || fail 'partial/mixed interrupted 0.1.8 install; residue retained'
    if [ -e "$JOURNAL" ] || [ -L "$JOURNAL" ]; then
        fail 'partial/mixed interrupted 0.1.8 install; residue retained'
    fi
    printf 'AgentPorter bootstrap: safely recovered interrupted 0.1.8 install\n' >&2
    exit 0
}
...
names=("agentporter","agentporter-activate","agentporter-uninstall")
expected={"schema_version":2,"product":"agentporter","version":"0.1.6","public_entries":[str(bin_home/name) for name in names]}
...
for name in names:
 ...
 if not public.is_symlink() or os.readlink(public) != str(private) or not private.is_file(): raise SystemExit(1)' \
        "$OLD_ROOT" "$BIN_HOME" || fail 'existing 0.1.6 installation is not safe to upgrade'
    UPGRADE=1
else
...
STAGING=$(mktemp -d "${PRODUCT_ROOT}/.0.1.8-stage.XXXXXX") || fail 'could not create a private staging directory'
...
names=("agentporter","agentporter-activate","agentporter-uninstall")
spec={"schema":2,"mode":str(mode),"old_root":str(old_root),"new_root":str(new_root),"old_receipt":str(old_root/"bootstrap-install.json"),"new_receipt":str(new_root/"bootstrap-install.json"),"journal":str(new_root.parent/".0.1.8-entry-transaction.json"),"entries":[]}
