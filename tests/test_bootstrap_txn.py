...
def _world(
    ...
    new.mkdir()
    if not fresh:
        (old / "bootstrap-install.json").write_text('{"version":"0.1.5"}\n')
    (new / "bootstrap-install.json").write_text('{"version":"0.1.6"}\n')
    bindir = tmp_path / "bin"
    bindir.mkdir()
    ...
    spec: dict[str, object] = {
        ...
        "entries": entries,
        "journal": str(tmp_path / ".0.1.6-entry-transaction.json"),
    }
...
def test_apply_owns_committed_old_root_cleanup_and_all_metadata(tmp_path: Path) -> None:
    ...
    assert not Path(str(spec["new_root"])).with_name(".0.1.5-txn-quarantine").exists()
    assert not Path(str(spec["journal"])).exists()
...
def test_uncommitted_recovery_owns_exact_new_root_cleanup(tmp_path: Path) -> None:
    ...
    assert not Path(str(spec["new_root"])).with_name(".0.1.6-txn-quarantine").exists()
    assert not Path(str(spec["journal"])).exists()
    assert not spec_path.exists()
